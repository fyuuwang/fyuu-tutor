...
- Portal publish wrapper: `scripts/publish_portal.py` reads a private `<workspace>/portal.toml` to determine which projects are eligible for publishing, runs all validators, builds the portal, scans for private data, and calls `deploy_portal.sh --publish`. After a course is produced and the claim is released, run this wrapper with `--publish` only with current-session user authority or documented project-level standing authorization; `publish_after_validation` defines eligibility, not authorization. If publishing fails, record the failure reason in STATUS.md without changing course status. Never bypass the wrapper to copy private directories into gh-pages.
